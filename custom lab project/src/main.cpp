...
uint8_t score = 0;
uint8_t EEMEM high_score = 0;
uint16_t EEMEM game_session_seed = 123;
uint8_t current_target_char_index;
uint8_t player_selected_char_index = 0;
volatile uint8_t auto_start_flag = 0;
...
int main(void) {
    ...
    _delay_ms(100); // A small delay to give the user time to press and hold the button.
    if ((PIND & (1 << CONFIRM_SEL_PIN)) == 0) { // Check if the confirm button is being held down.
        write_high_score_eeprom(0); // Reset the high score in EEPROM to 0.
        
        
        update_lcd_message("High Score", "Has Been Reset!");
        _delay_ms(2500); 
    }


    update_lcd_message("Morse Challenge!","");
    ...
    while (1) {
        ...
        switch (current_game_state) {
            case INIT_GAME:
                ...
                {
                    uint16_t seed = eeprom_read_word(&game_session_seed);
                    eeprom_update_word(&game_session_seed, seed + 1);
                    srand(seed + TCNT1);
                }
                if (auto_start_flag) {
                    auto_start_flag = 0;
                ...
                update_seven_segment_display(1, 1);
                char hs_str[17];
                sprintf(hs_str, "High Score: %02u", read_high_score_eeprom());
                update_lcd_message("MORSE CODE GAME", hs_str);
                break;
            ...
            case NEW_ROUND:
                ...
                char round_str[17];
                sprintf(round_str, "Round: %u/5", current_round);
            ...
            case GAME_OVER_SEQUENCE:
                { 
                    ...
                    if (score >= 4) { 
                        update_lcd_message("YOU WIN!", "Congrats!");
                    } else {
                        update_lcd_message("GAME OVER", "Try Again.");
                    }
                    _delay_ms(2000);
                    ...
                    if (score > current_hs) {
                        ...
                        current_hs = score; 
                        update_lcd_message("New High Score!", "");
                        _delay_ms(2000);
                    }
                    ...
                    sprintf(final_hs_msg, "High Score: %02u", current_hs);
...
void update_seven_segment_display(uint16_t value, uint8_t display_mode) {
    ...
    if (tens == 0) {
        seven_segment_digits[2] = SEG_BLANK;
    } else {
        seven_segment_digits[2] = tens;
    }

    seven_segment_digits[3] = ones;
}
...
void play_tone(uint16_t frequency, uint16_t duration_ms) {
    ...
    if (half_period_us == 0 && frequency > 0) half_period_us = 1; 
...
void play_morse_char(uint8_t char_index) {
    ...
    for (uint8_t i = 0; morse_str[i] != '\0'; i++) {
        lcd_write_character((morse_str[i] == '.') ? 0 : 1); 
...
uint8_t read_high_score_eeprom(void) {
    uint8_t score_val = eeprom_read_byte((uint8_t*)&high_score);
    if (score_val == 255) {
        eeprom_update_byte((uint8_t*)&high_score, 0); 
        return 0; 
    }
    return score_val;
}
...
void write_high_score_eeprom(uint8_t new_score) {
    eeprom_update_byte((uint8_t*)&high_score, new_score); 
}
