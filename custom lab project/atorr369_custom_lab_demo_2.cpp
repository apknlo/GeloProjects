...
uint16_t game_session_seed EEMEM = 123; 
    ...
    while (1) {
        ...
        switch (current_game_state) {
            case INIT_GAME:
                ...
                { 
                    uint16_t seed_from_eeprom = eeprom_read_word(&game_session_seed);
                    uint16_t next_seed_to_store = seed_from_eeprom + 1;
                    
                    eeprom_update_word(&game_session_seed, next_seed_to_store); 
                    srand(seed_from_eeprom + TCNT1); 
                }
                
                update_seven_segment_display(player_selected_char_index + 1, 1);
                update_lcd_message("MORSE CODE GAME", " ");
                break;
...
void handle_buttons(void) {
    ...
    if (confirm_debounce_counter > 0) {
        ...
        if (confirm_debounce_counter == 0 && current_confirm_val != confirm_button_state) {
