 ...
 *         Demo Link: https://youtu.be/O_W37XnU9qE
...
static const uint8_t patL_D[4] = { (1 << LEDL1), (1 << LEDL1) | (1 << LEDL2), (1 << LEDL1) | (1 << LEDL2), 0 };
static const uint8_t patL_B[4] = { (1 << LEDL3), (1 << LEDL3), (1 << LEDL3), 0 };
static const uint8_t patR_D[4] = { (1 << LEDR1), (1 << LEDR1) | (1 << LEDR2), (1 << LEDR1) | (1 << LEDR2) | (1 << LEDR3), 0 };
...
int TickF_StepperControl(int current_stepper_fsm_state) {
    ...
    long max_displacement_up = ADC_MAX_INPUT_VAL - (ADC_CENTER_VAL + ADC_DEADBAND);
    if (max_displacement_up <= 0) max_displacement_up = 1;

    long max_displacement_down = (ADC_CENTER_VAL - ADC_DEADBAND) - ADC_MIN_INPUT_VAL;
    if (max_displacement_down <= 0) max_displacement_down = 1;

    adc_val_x = ADC_read(JOYSTICK_X_ADC_CHANNEL);

    joystick_is_physically_down_for_brake = 0; 
    if (adc_val_x > (ADC_CENTER_VAL + ADC_DEADBAND)) { 
        current_stepper_fsm_state = SM_CW; 
        displacement_from_deadband_edge = adc_val_x - (ADC_CENTER_VAL + ADC_DEADBAND);
        if (displacement_from_deadband_edge > max_displacement_up) displacement_from_deadband_edge = max_displacement_up;
        
        if (max_displacement_up > 0) {
            long delta_delay_total = (long)MIN_STEP_DELAY_MS - (long)MAX_STEP_DELAY_MS; 
            long scaled_delta = (displacement_from_deadband_edge * delta_delay_total) / max_displacement_up;
    ...
    } else if (adc_val_x < (ADC_CENTER_VAL - ADC_DEADBAND)) { 
        current_stepper_fsm_state = SM_CCW; 
        displacement_from_deadband_edge = (ADC_CENTER_VAL - ADC_DEADBAND) - adc_val_x;
        if (displacement_from_deadband_edge > max_displacement_down) displacement_from_deadband_edge = max_displacement_down;

        if (max_displacement_down > 0) {
            long delta_delay_total = (long)MIN_STEP_DELAY_MS - (long)MAX_STEP_DELAY_MS; 
            long scaled_delta = (displacement_from_deadband_edge * delta_delay_total) / max_displacement_down;
            current_target_step_delay_ms = (unsigned long)((long)MAX_STEP_DELAY_MS + scaled_delta);
        } else { 
            current_target_step_delay_ms = MAX_STEP_DELAY_MS; 
        }
    ...
    if (motor_should_be_active && (step_timer_ms >= current_target_step_delay_ms)) {
        ...
        if (current_stepper_fsm_state == SM_CW) { 
            ...
            if (current_phase_index == 0) {
                current_phase_index = NUM_STEPPER_PHASES - 1;
            } else {
                current_phase_index--;
            }
        }
    ...
    switch (beeper_state) {
        case BEEP_FSM_IDLE:
            if (joystick_is_physically_down_for_brake) {
                beeper_state = BEEP_FSM_ACTIVE;
                OCR0A = BRAKE_BEEP_OCR_VAL;
                beeper_cycle_timer_ms = 0;
            }
            break;
        case BEEP_FSM_ACTIVE:
            if (beeper_cycle_timer_ms >= 1000) {
                OCR0A = SILENT_OCR_VAL;
                beeper_state = BEEP_FSM_SILENT_INTERVAL;
                beeper_cycle_timer_ms = 0;
            }
            break;
