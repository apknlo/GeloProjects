...
volatile unsigned char unitFlag  = 0;    // 0 = cm, 1 = inches
...
//── task periods ─────────────────────────────────────────────────────────────
const unsigned long TASK1_PERIOD = 1000;  // sonar read, ms
const unsigned long TASK2_PERIOD =    1;  // display multiplex, ms
const unsigned long TASK3_PERIOD =  200;  // left-button, ms
const unsigned long TASK4_PERIOD =    1;  // red PWM tick, ms
const unsigned long TASK5_PERIOD =    1;  // green PWM tick, ms
const unsigned long TASK6_PERIOD =  200;  // right-button, ms
const unsigned long TASK7_PERIOD =  250;  // blue blink tick, ms

//── GCD of all task periods ─────────────────────────────────────────────────
...
//── scheduler called every GCD_PERIOD ms ─────────────────────────────────────
void TimerISR() {
...
int main(void) {
    // configure I/O:
    // PC2=TRIG out, PC3=RED out, PC4=GREEN out, PC5=BLUE out
    // PC0=LEFT BTN, PC1=RIGHT BTN inputs w/ pull-ups
    DDRC  = (1<<SONAR_TRIG)|(1<<RED_PIN)|(1<<GREEN_PIN)|(1<<BLUE_PIN);
    PORTC = (1<<BTN_LEFT)|(1<<BTN_RIGHT);

    // PB1=SEG_G, PB2–PB5=DIG1–DIG4 outputs; PB0=ECHO input
    DDRB  = (1<<SEG_G)|(1<<DIG1)|(1<<DIG2)|(1<<DIG3)|(1<<DIG4);
    PORTB = (1<<SONAR_ECHO);

    // PD2–PD7 = SEG_A–SEG_F outputs
    ...
    // initialize dynamic thresholds to 10×4/5=8 and 10×6/5=12
...
//── Tick_Display (right-justified, leading zeros, clear segments) ───────────
...
int Tick_Blue(int state) {
    ...
    // count 250ms ticks; after ~12 ticks (3s), stop blinking
    if (++blinkTicks >= 12) {
        blinkActive = 0;
        blinkTicks  = 0;
        PORTC &= ~(1<<BLUE_PIN);
